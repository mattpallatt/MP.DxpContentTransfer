...
public sealed class CmaClient
{
    ...
    // GET the CMA representation of a content item by GUID.
    public Task<CmaResponse> GetManagementAsync(string baseUrl, string token, Guid guid, string purpose) =>
        SendAsync(HttpMethod.Get, ManagementUrl(baseUrl, guid), token, purpose);
