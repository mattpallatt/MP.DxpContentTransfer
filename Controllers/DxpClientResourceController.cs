...
// Serves the small client-side script that the admin SPA needs to render the settings page. The
// script is referenced by DxpAdminScriptMiddleware, which injects a <script> tag for it into every
// admin page. Served from a controller (rather than a packaged static asset) so the class library
// is self-contained — there is no host-side ClientResources folder to deploy.
[AllowAnonymous]
